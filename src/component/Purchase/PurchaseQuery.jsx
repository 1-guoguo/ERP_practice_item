...
function PurchaseQueryTables () {
  ...
  const edit = (record) => {
    ...
    if(record.type === 3)  {
      // 跳转到采购入库单
      navigate('/purchaseinstorage')
      // 发布消息到采购入库单
      PubSub.publish('purchaseInstorage', record.documentsNo)
      // 发布消息到标签栏
      const data = {'label': '采购入库单', 'path': '/purchaseinstorage'}
      PubSub.publish('purchaseInstorage', data)
    }
    // 采购退货
    if(record.type === 1)  {
      // 跳转到采购退货单
      ...
      // 发布消息到采购退货单
      PubSub.publish('purchaseOutstorage', record.documentsNo)
      // 发布消息到标签栏
      const data = {'label': '采购退货单', 'path': '/purchasereturn'}
      PubSub.publish('purchaseOutstorage', data)
    }
  }
  useEffect(()=>{
    // 进行ajax请求
    axios.get('http://127.0.0.1:8000/documents/queryPurchase').then(
      response=>{
  ...
  const onSearch = (value) => {
    ...
    .then(
        response=>{
            ...
            if(value===''){
                navigate('/purchasequery')
                setDataStatus(!dataStatus)
            }else{
                // 展示到页面上
                navigate(`/purchasequery?value=${value}`)
                // 请求到的数据进行展示
                setDataSource(response.data)
            }
        }
    ...
    <Content className='contentStyle'>
      ...
      <Form name="search" 
        ...
        <Row style={{marginLeft: 10}}>
          ...
          <Col span={4}  style={{marginLeft: 10}}>
            <Form.Item name='supplierUnit' label='供货单位'>
              <Input />
            </Form.Item>
          </Col>
          <Col span={4}  style={{marginLeft: 10}}>
            <Form.Item name='handerBy' label='经手人'>
              <Input />
            </Form.Item>
          </Col>
          <Col span={4}  style={{marginLeft: 10}}>
            <Form.Item name='inStorage' label='仓库'>
              <Input />
            </Form.Item>
          </Col>
        </Row>
        <Row style={{marginLeft: 10}}>
          <Col span={4} >
            <Form.Item name='returnUnit' label='退回单位'>
              <Input />
            </Form.Item>
          </Col>
...
export default PurchaseQueryTables;
