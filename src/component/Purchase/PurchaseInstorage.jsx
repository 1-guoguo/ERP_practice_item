...
// 子组件
function PurchaseInstorageTables () {
  const [tForm] = Form.useForm();
  const [dataSource, setDataSource] = useState([])  // 首页表格数据源
  const [orderData, setOrderData] = useState([])  // 订单表格数据源
  ...
  const orderColumns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return(
          <Button onClick={()=>clickOrder(record.id)}>选择</Button>
        )
      }
    }
  ]
  ...
  const clickOrder = (key) => {
    // 通过id获取到订单
    axios.post('http://127.0.0.1:8000/purchaseOrder/getId/', key, {headers: {'Content-Type': 'application/json'}}).then(
      response=>{
        ...
        const value = {
          ...
          'inStorage': response.data[0]['warehouseName'],
        }
        // 设置表单数据
        setTimeout(()=>{
  ...
  const onFinish = (values) => {
    ...
    formvalue.inStorageNumber = dataSource[0]['number']
    formvalue.account = dataSource[0]['account']
    const value = {...values, recordTime: dayjs(values.recordTime).format('YYYY-MM-DD'), ...formvalue}
    // 如果有单据编号，证明是修改的
    if (values.documents){
      axios.post('http://127.0.0.1:8000/inStorage/alter/', value, {headers: {'Content-Type': 'application/json'}}).then(
        res=>{
      ...
      axios.post('http://127.0.0.1:8000/inStorage/save/', value, {headers: {'Content-Type': 'application/json'}}).then(
        res=>{
          // 请求成功，路由监听，先不管，保存之后保存按钮消失不见变成新增，整页内容不可修改
          setButShow(false)
          setShow(true)
  ...
  useEffect(()=>{
    console.log('@@@')
    // 如果接受到数据
    PubSub.subscribe('purchaseInstorage', (_, documentsNo)=>{
      // 数据展示到表单
      console.log(documentsNo)
      // 通过id获取到订单
    axios.post('http://127.0.0.1:8000/documents/show/', documentsNo, {headers: {'Content-Type': 'application/json'}}).then(
      response=>{
        console.log("res",response.data)
        // 通过获得的goosId获得商品信息
        let goodsId = response.data[0]['goodsNo']
        ...
        axios.post('http://127.0.0.1:8000/store/searchValue/', params, {headers: {'Content-Type': 'application/json'},}).then(
          res=>{
            setDataSource([{...res.data[0], number: response.data[0]['inStorageNumber'], account: response.data[0]['number']}])
          }
        )
        const value = {
          'inStorageNo': response.data[0]['inStorageNo'],
          'sourceOrderNo': response.data[0]['sourceOrderNo'],
          'supplierUnit': response.data[0]['supplierUnit'],
          'orderStatus': response.data[0]['orderStatus'],
          'handerBy': response.data[0]['handerBy'],
          'inStorage': response.data[0]['inStorage'],
    ...
    <Content className='contentStyle'>
      <Form name="sale" 
            ...
            >
              <Button 
                ...
                onClick={()=>{setOpenOrder(true); axios.get('http://127.0.0.1:8000/purchaseOrder/getType/').then(response=>{setOrderData(response.data)})}}>选择入库订单</Button>
              <Row style={{marginLeft: 10}}> 
              <Col span={4} >
                <Form.Item name='inStorageNo' label='单据编号'>
              ...
              <Col span={4} style={{marginLeft: 10}}>
                <Form.Item name='supplierUnit' label='购买单位'  rules={[{required: true}]}>
                  <Input disabled={show}/>
                </Form.Item>
              </Col>
              <Col span={4} style={{marginLeft: 10}} >
              ...
              <Row>
              <Col span={4} style={{marginLeft: 10}}>
                <Form.Item name='inStorage' label='入库仓库'  rules={[{required: true}]}>
                  <Input disabled={show}/>
                </Form.Item>
                </Col>
              <Col span={4} style={{marginLeft: 10}}>
      ...
      <Row style={{marginBottom: '5px'}}>
            ...
            <Col span={2} offset={12}>
              {butShow?(<Button htmlType='submit' disabled={show}>保存</Button>):(<><Button onClick={()=>{setShow(false)}}>新增</Button><Button onClick={()=>{setShow(false);tForm.resetFields()}}>修改</Button></>)}
            </Col>
            <Col span={2}>
              <Button>关闭</Button>
            </Col>
...
export default PurchaseInstorageTables;
