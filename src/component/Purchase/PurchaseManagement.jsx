...
function SaleManagementTable () {
  ...
  const onClick = (id) => {
    ...
    setOpenModal(true)
    axios.post('http://127.0.0.1:8000/purchaseOrder/getId/', id, {headers: {'Content-Type': 'application/json'}}).then(
      res=>{
  ...
  useEffect(()=>{
    // 进行ajax请求
    axios.get('http://127.0.0.1:8000/purchaseOrder/getPurchaseOrder').then(
      response=>{
  ...
  const handleDelete = (key) => {
    console.log(key)
    axios.post(`http://127.0.0.1:8000/purchaseOrder/delete/`, key, {headers: {'Content-Type': 'application/json'},}).then(
        response=>{
            // 设置新的数据源
  ...
  const onSearch = (value) => {
    ...
    console.log(params)
    axios.post('http://127.0.0.1:8000/purchseOrder/searchValue/', params, {headers: {'Content-Type': 'application/json'},})
    .then(
        response=>{
            ...
            if(value===''){
                ...
                // 展示到页面上
                navigate(`/purchaseorder?value=${value}`)
                // 请求到的数据进行展示
                setDataSource(response.data)
            }
        }
  ...
  const onFinish = (values) => {
    ...
    const value = {...values, orderTime: dayjs(values.orderTime).format('YYYY-MM-DD'),deliveryDate: dayjs(values.deliveryDate).format('YYYY-MM-DD')}
    console.log(value)
    if (values.orderNo){
      axios.post('http://127.0.0.1:8000/purchaseOrder/alter/', value, {headers: {'Content-Type': 'application/json'},}).then(
        response=>{
          console.log(response)
          setDataStatus(!dataStatus)
          setOpen(false)
      ...
      console.log(value)
      axios.post('http://127.0.0.1:8000/purchaseOrder/addData/', value, {headers: {'Content-Type': 'application/json'},}).then(
        response=>{
          console.log(response)
          setDataStatus(!dataStatus)
          setOpen(false)
    ...
    <Content className='contentStyle'>
      <Row style={{marginBottom: '5px'}}>
            ...
            <Col span={3} offset={12}>
                <Button onClick={()=>{setOpen(true);setMake('新增采购订单');form.resetFields();setListData([])}}>新增采购订单</Button>
            </Col>
      </Row>
      {/* 首页列表 */}
      <Spin spinning={spinning}>
      <Table rowKey={(record)=>record.id} rowSelection={rowSelection} columns={columns} dataSource={dataSource} scroll={{x: 1500,}} />
      </Spin>
        ...
        >
          <Row style={{width: '100%',marginBottom: 10}}>
            ...
            <Form.Item
                  ...
                  name="orderNo"
              >
                <Input placeholder='采购订单编号自动生成' disabled/>
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item
