// 销售出库单
// 销售订单管理
import { Button, Modal, Tag, Table, Layout, Form, Row, Col, Input, message, DatePicker, Select  } from 'antd';
...
function SaleOutstorageTables () {
    ...
    },
    {
      ...
      render: (_,record)=>{
        ...
        if (record.orderStatus === 4){
          return <Tag color='green'>订单已确定</Tag>
        }
      }
    },
    {
  ...
  const clickOrder = (key) => {
    // 通过id获取到订单
    axios.post('http://127.0.0.1:8000/saleOrder/getId/', key, {headers: {'Content-Type': 'application/json'}}).then(
      response=>{
  ...
  const onFinish = (values) => {
    ...
    if (values.documents){
      ...
      axios.post('http://127.0.0.1:8000/outStorage/save/', value, {headers: {'Content-Type': 'application/json'}}).then(
        res=>{
          // 请求成功，路由监听，先不管，保存之后保存按钮消失不见变成新增，整页内容不可修改
          setButShow(false)
          setShow(true)
  ...
  const setOption = ()=>{
    axios.get('http://127.0.0.1:8000/customer/').then(
      res=>{
        // console.log(res.data)
        for (let item in res.data){
          // console.log("i", item)
          // console.log("name", res.data[item].customerName)
          const op = {value: item, label: res.data[item].customerName}
          setUnitOptions([...unitOptions, op])
  ...
  useEffect(()=>{
    console.log('@@@')
    // 如果接受到数据
    PubSub.subscribe('saleOutrage', (_, documentsNo)=>{
      // 数据展示到表单
    ...
    <Content className='contentStyle'>
      <Form name="sale" 
            ...
            >
              <Button 
                ...
                onClick={()=>{setOpenOrder(true); axios.get('http://127.0.0.1:8000/saleOrder/getType/').then(response=>{setOrderData(response.data)})}}>选择出库订单</Button>
              ...
              <Col span={4} style={{marginLeft: 10}}>
                <Form.Item name='purchaseUnit' label='购买单位'  rules={[{required: true}]}>
                  <Select onMouseEnter={setOption} onMouseLeave={()=>{setUnitOptions([])}} onChange={(value)=>{console.log(value)}} options={unitOptions} disabled={show} />
                  {/* <Input disabled={show}/> */}
                </Form.Item>
              </Col>
              <Col span={4} style={{marginLeft: 10}} >
              ...
              <Col span={4} style={{marginLeft: 10}}>
                <Form.Item name='documentsType' label='出库单据类型'  rules={[{required: true}]}>
      ...
      <Row style={{marginBottom: '5px'}}>
            ...
            <Col span={2} offset={12}>
              {butShow?(<Button htmlType='submit' disabled={show}>保存</Button>):(<><Button onClick={()=>{setShow(false);tForm.resetFields()}}>新增</Button> <Button onClick={()=>{setShow(false)}}>修改</Button></>)}
            </Col>
            <Col span={2}>
              <Button>关闭</Button>
            </Col>
...
export default SaleOutstorageTables;
