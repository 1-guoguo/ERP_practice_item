// 销售出库单
// 销售订单管理
import { Alert, Button, Modal, Tag, Table, Layout, Form, Row, Col, Input, message, InputNumber, Typography, DatePicker, Select  } from 'antd';
...
function SaleReturnTable () {
  ...
  const onFinish = (values) => {
    ...
    formvalue.inStorageNumber = dataSource[0]['number']  //销售退货=入库
    ...
    axios.post('http://127.0.0.1:8000/inStorage/saveReturn/', value, {headers: {'Content-Type': 'application/json'}}).then(
      res=>{
        // 请求成功，如果该商品不可退货，给出提醒
  ...
  const clickOrder = (key) => {
    // 通过单据获取到订单
    axios.post('http://127.0.0.1:8000/saleOrder/getMsg/', key, {headers: {'Content-Type': 'application/json'}}).then(
      response=>{
  ...
  useEffect(()=>{
    console.log('@@@')
    // 如果接受到数据
    PubSub.subscribe('saleInstorage', (_, documentsNo)=>{
      // 数据展示到表单
    ...
    axios.post('http://127.0.0.1:8000/documents/show/', documentsNo, {headers: {'Content-Type': 'application/json'}}).then(
      response=>{
        ...
        const value = {
          ...
          'purchaseUnit': response.data[0]['returnUnit'],
          'orderStatus': response.data[0]['orderStatus'],
          'handerBy': response.data[0]['handerBy'],
          'outStorage': response.data[0]['inStorage'],
      ...
      <Form name="sale" 
            ...
            >
              <Button 
                ...
                onClick={()=>{setOpenOrder(true); axios.get('http://127.0.0.1:8000/documents/getOut/').then(response=>{setOrderData(response.data)})}}>选择退货订单</Button>
            <Row style={{marginLeft: 10}}>
              ...
              <Col span={4} style={{marginLeft: 10}} rules={[{required: true}]}>
                <Form.Item name='sourceOrderNo' label='关联订单编号'>
                  <Input disabled={show} />
                </Form.Item>
              </Col>
              <Col span={4} style={{marginLeft: 10}}>
                <Form.Item name='purchaseUnit' label='退货单位'  rules={[{required: true}]}>
                  <Input disabled={show}/>
                </Form.Item>
              </Col>
              <Col span={4} style={{marginLeft: 10}} >
              ...
              <Row>
              <Col span={4} style={{marginLeft: 10}}>
                <Form.Item name='outStorage' label='入库仓库'  rules={[{required: true}]}>
                  <Input disabled={show}/>
                </Form.Item>
                </Col>
              <Col span={4} style={{marginLeft: 10}}>
